...
export default function Mapper() {
  ...
  const handleShowRoute = () => {
    setRouteIndex(0); // Reset the route index when showing a new route
  };
      ...
      <div>
        {markerPositions.map((_, index) => (
          <button
            ...
            onClick={() => togglePointSelection(index)}
            style={{
              backgroundColor: selectedPoints.includes(index) ? "green" : "gray",
            }}
          >
...
function RouteSummary({ routes, routeIndex, setRouteIndex }) {
  ...
  const totalDistance = route.legs.reduce((sum: any, leg: { distance: { value: any; }; }) => sum + leg.distance.value, 0);
  const totalDuration = route.legs.reduce((sum: any, leg: { duration: { value: any; }; }) => sum + leg.duration.value, 0);
